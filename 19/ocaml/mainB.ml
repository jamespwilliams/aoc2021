...
let manhattan (ax, ay, az) (bx, by, bz) =
...
let match_reports : point list -> point list -> (point * point list) option =
 fun r1 r2 ->
...
let () =
  ...
  let offsets = ref [ (0, 0, 0) ] in
  let reports = ref (List.tl_exn reports) in

  while List.length !reports > 0 do
    let match_index, (matched_offset, matched_report) =
    ...
    offsets := matched_offset :: !offsets;

    reports :=
      List.filteri !reports ~f:(fun index _ -> not (index = match_index))
  done;

  List.concat_map !offsets ~f:(fun o1 ->
      List.map !offsets ~f:(fun o2 -> manhattan o1 o2))
  |> List.max_elt ~compare |> Option.value_exn |> Printf.printf "%d\n"
