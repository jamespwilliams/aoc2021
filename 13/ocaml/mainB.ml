...
let render_points max_x max_y points =
  let paper = Array.init max_y ~f:(fun _ -> Array.create ~len:max_x ".") in
  List.iter points ~f:(fun (x, y) -> paper.(y).(x) <- "#");
  String.concat ~sep:"\n"
    (Array.map paper ~f:(fun row -> String.concat (Array.to_list row))
    |> Array.to_list)

let () =
  List.fold folds ~init:points ~f:(fun points fold -> do_fold fold points)
  |> render_points 100 100 |> print_endline
